...
2) In this `bot/` folder:  
   ...
   - Set env:  
     ...
     - Optional: `DATA_DIR=/app/data` — mount a Railway volume (or any persistent disk) here so guild config and premium status survive redeploys instead of living only in the git working directory. Without this, both files are read/written next to the bot's own code and are lost on any redeploy that doesn't preserve the filesystem.
     - Optional: `GUILD_CONFIG_PATH=<path>` / `PREMIUM_CONFIG_PATH=<path>` to override each file's location individually instead of using `DATA_DIR` for both.
