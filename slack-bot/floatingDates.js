...
// {month, day} for a given year. Mirrors assets/floating-dates.js on the
// website exactly (same verified seasonal-marker table, sourced from
// astropixels.com/ephemeris/soleq2001.html and converted UTC->US-Eastern
// calendar date) — keep the two in sync if either changes.

const SEASONAL_MARKERS = {
...
function resolveSeasonalMarker(year, eventKey) {
  const row = SEASONAL_MARKERS[year];
  if (!row || !eventKey) return null;
  const idx = EVENT_INDEX[eventKey];
  if (idx === undefined) return null;
  const [month, day] = row[idx];
  return { month, day };
}
...
function resolveNthWeekday(year, month, weekday, ordinal) {
  if (ordinal === -1) {
    const last = new Date(year, month, 0); // day 0 of "next" month = last day of `month`
    const diff = (last.getDay() - weekday + 7) % 7;
    return { month, day: last.getDate() - diff };
  }
  const first = new Date(year, month - 1, 1);
  const offset = (weekday - first.getDay() + 7) % 7;
  return { month, day: 1 + offset + (ordinal - 1) * 7 };
}

function resolveRelativeToEvent(year, event, weekday, direction) {
  const anchor = resolveSeasonalMarker(year, event);
  if (!anchor) return null;
  const d = new Date(year, anchor.month - 1, anchor.day);
...
export function resolveDateRule(dateRule, year) {
